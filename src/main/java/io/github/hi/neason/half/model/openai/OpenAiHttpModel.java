...
/** 两种 OpenAI HTTP 协议共用的传输、订阅与生命周期；不解释供应商 JSON。 */
abstract class OpenAiHttpModel implements ChatModel, AutoCloseable {
    ...
    public Flow.Publisher<ModelEvent> stream(ChatRequest request) {
        ...
        return subscriber -> {
            ...
            var stream = new OpenAiStream(newEventDecoder(), subscriber, current -> startStream(request, current), active::remove);
            active.add(stream);
    ...
    abstract String encodeRequest(ChatRequest request, boolean streaming) throws IOException;
    abstract ChatResponse decodeResponse(String body) throws ModelProtocolException;
    abstract OpenAiStream.EventDecoder newEventDecoder();
