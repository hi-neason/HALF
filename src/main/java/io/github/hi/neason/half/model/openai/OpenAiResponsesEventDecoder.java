...
final class OpenAiResponsesEventDecoder {
    ...
    List<ModelEvent> accept(String data) throws ModelProtocolException {
        ...
        switch (type) {
            ...
            case "response.output_item.added" -> {
                ...
                switch (kind) {
                    ...
                    case "function_call" -> {
                        ...
                        String initial = text(value, "arguments");
                        if (!initial.isEmpty()) {
            ...
            case "response.content_part.added" -> {
                ...
                if (!"output_text".equals(text(part, "type")) || !text(part, "text").isEmpty()
                        || item.parts.size() >= 128 || partIndex != item.parts.size()) throw invalid("Invalid text part start");
                item.parts.put(partIndex, new Part());
            }
            case "response.output_text.delta" -> {
                ...
                if (!delta.isEmpty()) events.add(new ModelEvent.TextDelta(delta));
            }
            case "response.output_text.done" -> {
                Part part = part(root);
                if (part.textDone || part.done || !part.text.toString().equals(text(root, "text"))) throw invalid("Text completion does not match deltas");
                part.textDone = true;
            }
            case "response.content_part.done" -> {
                Part part = part(root);
                JsonNode value = root.path("part");
                if (part.done || !part.textDone || !"output_text".equals(text(value, "type"))
                        || !part.text.toString().equals(text(value, "text"))) throw invalid("Invalid text part completion");
                part.done = true;
            }
            ...
            case "response.reasoning_summary_part.added", "response.reasoning_summary_part.done",
                 ...
                 "response.reasoning_text.delta", "response.reasoning_text.done" -> { item(root, "reasoning"); }
    ...
    private void validateItem(Item item, JsonNode value) throws ModelProtocolException {
        ...
        if (!item.content().equals(decoded)) throw invalid("Output item does not match streamed content");
    }
    ...
    private static final class Item {
        ...
        List<ContentBlock> content() {
            ...
            return parts.values().stream().<ContentBlock>map(p -> new ContentBlock.Text(p.text.toString())).toList();
        }
    }
}
