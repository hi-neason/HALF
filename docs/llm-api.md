...
当前实现的是 OpenAI Chat Completions 协议的非流式文本子集。一个兼容服务可使用同一适配器并配置不同的完整接口 URL、密钥和模型 ID，但必须实际支持所发送的字段。
...
协议允许 `content: null`，例如出现工具调用、拒绝或过滤结果。当前抽象仅承载纯文本：工具调用、非 null 的 `refusal`、缺失或非字符串的内容均抛出 `ModelProtocolException`，表示当前实现无法承载此响应，而不意味着服务端一定违反协议。后续应扩展响应类型，分别表达文本、拒绝与工具调用。
...
3. [OpenAiChatModel](../src/main/java/io/github/hi/neason/half/model/openai/OpenAiChatModel.java)：`encodeRequest` → `HttpClient.send` → `decodeResponse`。
...
`OpenAiChatModel` 自己管理可复用的 `HttpClient`，使用完毕后关闭；示例通过 try-with-resources 完成释放。端点必须是完整 HTTP(S) URL，不包含 URL 用户凭据、查询参数或 fragment。远程服务使用 HTTPS，本地测试使用 HTTP。
