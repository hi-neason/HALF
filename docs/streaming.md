...
模型流与原始 API 流共用 SSE 的 `event` / `data` 字段解析，但保持各自的大小限制：模型流每帧累计至多 1 Mi 个行内字符，不计 JDK 已移除的换行符；原始 API 流通过 `SseFrameReader` 限制为 4 Mi 个字符，包含行结束符（CRLF 按一个计）。原始流逐字符检查，避免先读取超长行再检查大小。
